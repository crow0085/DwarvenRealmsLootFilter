export const STAT_REGISTRY = {
  // ---------------------------------------------------------------------------
  // PRIMARY ATTRIBUTES
  // ---------------------------------------------------------------------------
  strength: {
    id: "strength",
    name: "Strength",
    category: "attributes",
    patterns: ["Characteristics.Strength", "Strength"],
    canonical: "Characteristics.Strength",
    isPercent: false,
    format: (v: number) => `+${v.toFixed(0)}`,
    description: "Strength",
  },
  strengthBonus: {
    id: "strengthBonus",
    name: "Strength Bonus",
    category: "attributes",
    patterns: [
      "Characteristics.Strength%6",
      "Characteristics.Strength%",
      "Strength%6",
      "Strength%",
    ],
    canonical: "Characteristics.Strength%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Strength Bonus",
  },
  dexterity: {
    id: "dexterity",
    name: "Dexterity",
    category: "attributes",
    patterns: ["Characteristics.Dexterity", "Dexterity"],
    canonical: "Characteristics.Dexterity",
    isPercent: false,
    format: (v: number) => `+${v.toFixed(0)}`,
    description: "Dexterity",
  },
  dexterityBonus: {
    id: "dexterityBonus",
    name: "Dexterity Bonus",
    category: "attributes",
    patterns: [
      "Characteristics.Dexterity%6",
      "Characteristics.Dexterity%",
      "Dexterity%6",
      "Dexterity%",
    ],
    canonical: "Characteristics.Dexterity%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Dexterity Bonus",
  },
  wisdom: {
    id: "wisdom",
    name: "Wisdom",
    category: "attributes",
    patterns: ["Characteristics.Wisdom", "Wisdom"],
    canonical: "Characteristics.Wisdom",
    isPercent: false,
    format: (v: number) => `+${v.toFixed(0)}`,
    description: "Wisdom",
  },
  wisdomBonus: {
    id: "wisdomBonus",
    name: "Wisdom Bonus",
    category: "attributes",
    patterns: [
      "Characteristics.Wisdom%6",
      "Characteristics.Wisdom%",
      "Wisdom%6",
      "Wisdom%",
    ],
    canonical: "Characteristics.Wisdom%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Wisdom Bonus",
  },
  luck: {
    id: "luck",
    name: "Luck",
    category: "attributes",
    patterns: ["Characteristics.Luck", "Luck"],
    canonical: "Characteristics.Luck",
    isPercent: false,
    format: (v: number) => `+${v.toFixed(0)}`,
    description: "Luck",
  },
  luckBonus: {
    id: "luckBonus",
    name: "Luck Bonus",
    category: "attributes",
    patterns: [
      "Characteristics.Luck%6",
      "Characteristics.Luck%",
      "Luck%6",
      "Luck%",
    ],
    canonical: "Characteristics.Luck%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Luck Bonus",
  },
  agility: {
    id: "agility",
    name: "Agility",
    category: "attributes",
    patterns: ["Characteristics.Agility", "Agility"],
    canonical: "Characteristics.Agility",
    isPercent: false,
    format: (v: number) => `+${v.toFixed(0)}`,
    description: "Agility",
  },
  agilityBonus: {
    id: "agilityBonus",
    name: "Agility Bonus",
    category: "attributes",
    patterns: [
      "Characteristics.Agility%6",
      "Characteristics.Agility%",
      "Agility%6",
      "Agility%",
    ],
    canonical: "Characteristics.Agility%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Agility Bonus",
  },
  endurance: {
    id: "endurance",
    name: "Endurance",
    category: "attributes",
    patterns: [
      "Characteristics.Endurance",
      "Endurance",
      "Characteristics.Intelligence", // Game uses Intelligence internally
      "Intelligence",
    ],
    canonical: "Characteristics.Intelligence",
    isPercent: false,
    format: (v: number) => `+${v.toFixed(0)}`,
    description: "Endurance",
  },
  enduranceBonus: {
    id: "enduranceBonus",
    name: "Endurance Bonus",
    category: "attributes",
    patterns: [
      "Characteristics.Intelligence%6",
      "Characteristics.Intelligence%",
      "Characteristics.Endurance%6",
      "Characteristics.Endurance%",
      "Intelligence%6",
      "Intelligence%",
      "Endurance%6",
      "Endurance%",
    ],
    canonical: "Characteristics.Intelligence%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Endurance Bonus",
  },
  stamina: {
    id: "stamina",
    name: "Stamina",
    category: "attributes",
    patterns: ["Characteristics.Stamina", "Stamina"],
    canonical: "Characteristics.Stamina",
    isPercent: false,
    format: (v: number) => `+${v.toFixed(0)}`,
    description: "Stamina",
  },
  staminaBonus: {
    id: "staminaBonus",
    name: "Stamina Bonus",
    category: "attributes",
    patterns: [
      "Characteristics.Stamina%6",
      "Characteristics.Stamina%",
      "Stamina%6",
      "Stamina%",
    ],
    canonical: "Characteristics.Stamina%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Stamina Bonus",
  },

  // ---------------------------------------------------------------------------
  // OFFENSE - GENERIC
  // ---------------------------------------------------------------------------
  // damage: {
  //   id: "damage",
  //   name: "Damage",
  //   category: "offense",
  //   patterns: ["Damage"],
  //   canonical: "Damage",
  //   isPercent: false,
  //   format: (v: number) => `+${v.toFixed(0)}`,
  //   description: "Base damage",
  //   // Use $ anchor in regex to avoid matching sub-patterns
  //   regexPatterns: ["Damage$"],
  // },
  // damageBonus: {
  //   id: "damageBonus",
  //   name: "Damage Bonus",
  //   category: "offense",
  //   patterns: ["Base.Damage%6", "Damage%6", "Damage%"],
  //   canonical: "Base.Damage%6",
  //   isPercent: true,
  //   format: (v: number) => `+${(v * 100).toFixed(0)}%`,
  //   description: "Damage Bonus",
  // },
  // damageMultiplier: {
  //   id: "damageMultiplier",
  //   name: "Damage Multiplier",
  //   category: "offense",
  //   patterns: ["DamageMultiplier"],
  //   canonical: "DamageMultiplier",
  //   isPercent: true,
  //   format: (v: number) => `+${(v * 100).toFixed(0)}%`,
  //   description: "Generic damage multiplier (ability-specific variant unknown)",
  //   displayOnly: true,
  // },
  critChance: {
    id: "critChance",
    name: "Critical Chance",
    category: "offense",
    patterns: ["CriticalChance", "CritChance", "CriticalChance%"],
    canonical: "CriticalChance",
    isPercent: true,
    format: (v: number) => `${(v * 100).toFixed(1)}%`,
    description: "Chance to deal critical strikes",
  },
  critDamage: {
    id: "critDamage",
    name: "Critical Damage",
    category: "offense",
    patterns: ["CriticalDamage%", "CriticalDamage"],
    canonical: "CriticalDamage%",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Bonus damage on critical strikes",
  },
  attackSpeed: {
    id: "attackSpeed",
    name: "Attack Speed",
    category: "offense",
    patterns: ["AttackSpeed"],
    canonical: "AttackSpeed",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Increased attack speed",
    regexPatterns: ["AttackSpeed", "Attack.*Speed"],
  },
  bossBonus: {
    id: "bossBonus",
    name: "Boss Damage Bonus",
    category: "offense",
    patterns: ["Damage.Boss%6", "Boss%6", "Boss%"],
    canonical: "Damage.Boss%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Bonus damage against bosses",
  },

  // ---------------------------------------------------------------------------
  // OFFENSE - STANCE/WEAPON SPECIFIC
  // ---------------------------------------------------------------------------
  mageryDamage: {
    id: "mageryDamage",
    name: "Magery Damage",
    category: "stance",
    patterns: [
      "Damage.Magery.Damage%6",
      "Magery.Damage%6",
      "MageryDamage%6",
      "MageryDamage",
      "MageryDamage%",
      "Magery.Damage",
    ],
    canonical: "Damage.Magery.Damage%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Bonus magical damage from all sources",
  },
  mageryCritDamage: {
    id: "mageryCritDamage",
    name: "Magery Critical Damage",
    category: "stance",
    patterns: [
      "Damage.Magery.CriticalDamage%6",
      "Magery.CriticalDamage%6",
      "MageryCriticalDamage%6",
      "MageryCriticalDamage%",
      "MageryCritDamage",
    ],
    canonical: "Damage.Magery.CriticalDamage%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Magery critical damage bonus",
  },
  mageryCritChance: {
    id: "mageryCritChance",
    name: "Magery Critical Chance",
    category: "stance",
    patterns: [
      "Damage.Magery.CriticalChance%6",
      "Magery.CriticalChance%6",
      "MageryCriticalChance%6",
      "MageryCriticalChance%",
      "MageryCritChance",
    ],
    canonical: "Damage.Magery.CriticalChance%6",
    isPercent: true,
    format: (v: number) => `${(v * 100).toFixed(1)}%`,
    description: "Magery critical chance bonus",
  },

  maulDamage: {
    id: "maulDamage",
    name: "Maul Damage",
    category: "stance",
    patterns: [
      "Damage.Mauls.Damage%",
      "Mauls.Damage%",
      "MaulsDamage%",
      "MaulDamage",
      "Mauls.Damage",
    ],
    canonical: "Damage.Mauls.Damage%",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Maul damage bonus",
  },
  maulCritDamage: {
    id: "maulCritDamage",
    name: "Maul Critical Damage",
    category: "stance",
    patterns: [
      "Damage.Mauls.CriticalDamage%",
      "Mauls.CriticalDamage%",
      "MaulsCriticalDamage%",
      "MaulCritDamage",
    ],
    canonical: "Damage.Mauls.CriticalDamage%",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Maul critical damage bonus",
  },
  maulCritChance: {
    id: "maulCritChance",
    name: "Maul Critical Chance",
    category: "stance",
    patterns: [
      "Damage.Mauls.CriticalChance%",
      "Mauls.CriticalChance%",
      "MaulsCriticalChance%",
      "MaulCritChance",
    ],
    canonical: "Damage.Mauls.CriticalChance%",
    isPercent: true,
    format: (v: number) => `${(v * 100).toFixed(1)}%`,
    description: "Maul critical chance bonus",
  },

  archeryDamage: {
    id: "archeryDamage",
    name: "Archery Damage",
    category: "stance",
    patterns: [
      "Damage.Archery.Damage%6",
      "Archery.Damage%6",
      "ArcheryDamage%6",
      "ArcheryDamage%",
      "ArcheryDamage",
      "Archery.Damage",
    ],
    canonical: "Damage.Archery.Damage%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Archery damage bonus",
  },
  archeryCritDamage: {
    id: "archeryCritDamage",
    name: "Archery Critical Damage",
    category: "stance",
    patterns: [
      "Damage.Archery.CriticalDamage%6",
      "Archery.CriticalDamage%6",
      "ArcheryCriticalDamage%6",
      "ArcheryCriticalDamage%",
      "ArcheryCritDamage",
    ],
    canonical: "Damage.Archery.CriticalDamage%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Archery critical damage bonus",
  },
  archeryCritChance: {
    id: "archeryCritChance",
    name: "Archery Critical Chance",
    category: "stance",
    patterns: [
      "Damage.Archery.CriticalChance%6",
      "Archery.CriticalChance%6",
      "ArcheryCriticalChance%6",
      "ArcheryCriticalChance%",
      "ArcheryCritChance",
    ],
    canonical: "Damage.Archery.CriticalChance%6",
    isPercent: true,
    format: (v: number) => `${(v * 100).toFixed(1)}%`,
    description: "Archery critical chance bonus",
  },

  unarmedDamage: {
    id: "unarmedDamage",
    name: "Fists Damage",
    category: "stance",
    patterns: [
      "Damage.Fists.Damage%6",
      "Fists.Damage%6",
      "FistsDamage%6",
      "FistsDamage",
      "FistsDamage%",
      "FistsDamage%6",
      "UnarmedDamage%",
      "UnarmedDamage",
      "Unarmed.Damage",
    ],
    canonical: "Damage.Fists.Damage%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Fists/Unarmed damage bonus",
  },
  unarmedCritDamage: {
    id: "unarmedCritDamage",
    name: "Fists Critical Damage",
    category: "stance",
    patterns: [
      "Damage.Fists.CriticalDamage%6",
      "Fists.CriticalDamage%6",
      "FistsCriticalDamage%6",
      "UnarmedCriticalDamage%",
      "UnarmedCritDamage",
      "FistsCriticalDamage",
      "FistsCriticalDamage%",
      "FistsCriticalDamage%6",
    ],
    canonical: "Damage.Fists.CriticalDamage%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Fists/Unarmed critical damage bonus",
  },
  unarmedCritChance: {
    id: "unarmedCritChance",
    name: "Fists Critical Chance",
    category: "stance",
    patterns: [
      "Damage.Fists.CriticalChance%6",
      "Fists.CriticalChance%6",
      "FistsCriticalChance%6",
      "UnarmedCriticalChance%",
      "UnarmedCritChance",
      "FistsCriticalChance",
      "FistsCriticalChance%",
      "FistsCriticalChance%6",
    ],
    canonical: "Damage.Fists.CriticalChance%6",
    isPercent: true,
    format: (v: number) => `${(v * 100).toFixed(1)}%`,
    description: "Fists/Unarmed critical chance bonus",
  },

  swordDamage: {
    id: "swordDamage",
    name: "Swords Damage",
    category: "stance",
    patterns: [
      "Damage.OneHanded%6",
      "OneHanded%6",
      "SwordDamage%6",
      "SwordDamage%",
      "SwordDamage",
      "Sword.Damage",
      "Damage.OneHanded",
      "OneHanded",
      "OneHandDamage",
      "OneHandDamage%",
      "OneHandDamage%6",
    ],
    canonical: "Damage.OneHanded%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Swords damage bonus",
  },
  swordCritDamage: {
    id: "swordCritDamage",
    name: "Swords Critical Damage",
    category: "stance",
    patterns: [
      "Damage.OneHandCritcalDamage%6", // Game typo: "Critcal" not "Critical"
      "Damage.OneHandCritcalDamage%",
      "Damage.OneHandedCriticalDamage%6",
      "Damage.OneHandedCriticalDamage%",
      "OneHandCritcalDamage%6",
      "OneHandCritcalDamage%",
      "OneHandedCriticalDamage%6",
      "OneHandedCriticalDamage%",
      "SwordCriticalDamage%",
      "SwordCritDamage",
      "Damage.OneHandedCriticalDamage",
      "OneHandedCriticalDamage",
      "OneHandCriticalDamage",
      "OneHandCriticalDamage%",
      "OneHandCriticalDamage%6",
    ],
    canonical: "Damage.OneHandCritcalDamage%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Swords critical damage bonus",
  },
  swordCritChance: {
    id: "swordCritChance",
    name: "Swords Critical Chance",
    category: "stance",
    patterns: [
      "Damage.OneHandedCritcalChance%6", // Game typo: "Critcal" not "Critical"
      "Damage.OneHandedCritcalChance%",
      "Damage.OneHandedCriticalChance%6",
      "Damage.OneHandedCriticalChance%",
      "OneHandedCritcalChance%6",
      "OneHandedCritcalChance%",
      "OneHandedCriticalChance%6",
      "OneHandedCriticalChance%",
      "SwordCriticalChance%",
      "SwordCritChance",
      "Damage.OneHandedCriticalChance",
      "OneHandedCriticalChance",
      "OneHandCriticalChance",
      "OneHandCriticalChance%",
      "OneHandCriticalChance%6",
    ],
    canonical: "Damage.OneHandedCritcalChance%6",
    isPercent: true,
    format: (v: number) => `${(v * 100).toFixed(1)}%`,
    description: "Swords critical chance bonus",
  },

  spearDamage: {
    id: "spearDamage",
    name: "Spear Damage",
    category: "stance",
    patterns: [
      "Damage.Spear%6",
      "Damage.Spear%",
      "Spear%6",
      "Spear%",
      "SpearDamage%6",
      "SpearDamage%",
      "SpearDamage",
      "Spear.Damage",
      "SpearsDamage%6",
      "SpearsDamage%",
      "SpearsDamage",
      "Spears.Damage",
      "Damage.PoleArm%6", // PoleArm = Spear in game data
      "Damage.PoleArm%",
      "PoleArm%6",
      "PoleArm%",
    ],
    canonical: "Damage.Spear%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Spear damage bonus",
  },
  spearCritDamage: {
    id: "spearCritDamage",
    name: "Spear Critical Damage",
    category: "stance",
    patterns: [
      "Damage.SpearCritcalDamage%6", // Game typo: "Critcal" not "Critical"
      "Damage.SpearCritcalDamage%",
      "Damage.SpearCriticalDamage%6",
      "Damage.SpearCriticalDamage%",
      "SpearCritcalDamage%6",
      "SpearCritcalDamage%",
      "SpearCriticalDamage%6",
      "SpearCriticalDamage%",
      "SpearCritDamage",
      "SpearsCritcalDamage%6",
      "SpearsCritcalDamage%",
      "SpearsCriticalDamage%6",
      "SpearsCriticalDamage%",
      "SpearsCritDamage",
      "Damage.PoleArmCriticalDamage%6", // PoleArm = Spear in game data
      "Damage.PoleArmCriticalDamage%",
      "PoleArmCriticalDamage%6",
      "PoleArmCriticalDamage%",
      "Damage.PoleArmCritcalDamage%6", // Game typo: "Critcal" not "Critical"
      "Damage.PoleArmCritcalDamage%",
      "PoleArmCritcalDamage%",
    ],
    canonical: "Damage.SpearCritcalDamage%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Spear critical damage bonus",
  },
  spearCritChance: {
    id: "spearCritChance",
    name: "Spear Critical Chance",
    category: "stance",
    patterns: [
      "Damage.SpearCritcalChance%6", // Game typo: "Critcal" not "Critical"
      "Damage.SpearCritcalChance%",
      "Damage.SpearCriticalChance%6",
      "Damage.SpearCriticalChance%",
      "SpearCritcalChance%6",
      "SpearCritcalChance%",
      "SpearCriticalChance%6",
      "SpearCriticalChance%",
      "SpearCritChance",
      "SpearsCritcalChance%",
      "SpearsCriticalChance%6",
      "SpearsCriticalChance%",
      "SpearsCritChance",
      "Damage.PoleArmCriticalChance%6", // PoleArm = Spear in game data
      "Damage.PoleArmCriticalChance%",
      "PoleArmCriticalChance%6",
      "PoleArmCriticalChance%",
      "Damage.PoleArmCritcalChance%6", // Game typo: "Critcal" not "Critical"
      "Damage.PoleArmCritcalChance%",
      "PoleArmCritcalChance%",
    ],
    canonical: "Damage.SpearCritcalChance%6",
    isPercent: true,
    format: (v: number) => `${(v * 100).toFixed(1)}%`,
    description: "Spear critical chance bonus",
  },

  scytheDamage: {
    id: "scytheDamage",
    name: "Scythe Damage",
    category: "stance",
    patterns: [
      "Damage.Scythe.Damage%6",
      "Scythe.Damage%6",
      "ScytheDamage%6",
      "ScythesDamage%",
      "ScythesDamage",
      "Scythes.Damage",
    ],
    canonical: "Damage.Scythe.Damage%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Scythe damage bonus",
  },
  scytheCritDamage: {
    id: "scytheCritDamage",
    name: "Scythe Critical Damage",
    category: "stance",
    patterns: [
      "Damage.Scythe.CriticalDamage%6",
      "Scythe.CriticalDamage%6",
      "ScytheCriticalDamage%6",
      "ScythesCriticalDamage%",
      "ScythesCritDamage",
    ],
    canonical: "Damage.Scythe.CriticalDamage%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Scythe critical damage bonus",
  },
  scytheCritChance: {
    id: "scytheCritChance",
    name: "Scythe Critical Chance",
    category: "stance",
    patterns: [
      "Damage.Scythe.CriticalChance%6",
      "Scythe.CriticalChance%6",
      "ScytheCriticalChance%6",
      "ScythesCriticalChance%",
      "ScythesCritChance",
    ],
    canonical: "Damage.Scythe.CriticalChance%6",
    isPercent: true,
    format: (v: number) => `${(v * 100).toFixed(1)}%`,
    description: "Scythe critical chance bonus",
  },

  twohandDamage: {
    id: "twohandDamage",
    name: "Axes Damage",
    category: "stance",
    patterns: [
      "Damage.TwoHanded%6",
      "Damage.TwoHanded%",
      "TwoHanded%6",
      "TwoHanded%",
      "TwoHandedDamage%6",
      "TwoHandedDamage%",
      "TwoHandDamage%6",
      "TwoHandDamage%",
      "TwoHandedDamage",
      "TwoHandDamage",
      "Damage.TwoHandedDamage",
    ],
    canonical: "Damage.TwoHanded%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Axes damage bonus",
  },
  twohandCritDamage: {
    id: "twohandCritDamage",
    name: "Axes Critical Damage",
    category: "stance",
    patterns: [
      "Damage.TwoHandCritcalDamage%6", // Game typo: "Critcal" not "Critical"
      "Damage.TwoHandCritcalDamage%",
      "Damage.TwoHandedCriticalDamage%6",
      "Damage.TwoHandedCriticalDamage%",
      "TwoHandCritcalDamage%6",
      "TwoHandCritcalDamage%",
      "TwoHandCriticalDamage%",
      "TwoHandCriticalDamage%6",
      "TwoHandedCriticalDamage%6",
      "TwoHandedCriticalDamage%",
      "TwoHandedCritDamage",
      "TwoHandedCriticalDamage",
    ],
    canonical: "Damage.TwoHandCritcalDamage%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Axes critical damage bonus",
  },
  twohandCritChance: {
    id: "twohandCritChance",
    name: "Axes Critical Chance",
    category: "stance",
    patterns: [
      "Damage.TwoHandCritcalChance%6", // Game typo: "Critcal" not "Critical"
      "Damage.TwoHandCritcalChance%",
      "Damage.TwoHandedCriticalChance%6",
      "Damage.TwoHandedCriticalChance%",
      "TwoHandCritcalChance%6",
      "TwoHandCritcalChance%",
      "TwoHandCriticalChance%6",
      "TwoHandCriticalChance%",
      "TwoHandedCriticalChance%6",
      "TwoHandedCriticalChance%",
      "TwoHandedCriticalChance",
      "TwoHandedCritChance",
    ],
    canonical: "Damage.TwoHandCritcalChance%6",
    isPercent: true,
    format: (v: number) => `${(v * 100).toFixed(1)}%`,
    description: "Axes critical chance bonus",
  },

  // ---------------------------------------------------------------------------
  // DEFENSE
  // ---------------------------------------------------------------------------
  // armor: {
  //   id: "armor",
  //   name: "Armor",
  //   category: "defense",
  //   patterns: ["Armor", "armor"],
  //   canonical: "Armor",
  //   isPercent: false,
  //   format: (v: number) => v.toFixed(0),
  //   description: "Armor",
  //   regexPatterns: ["Armor$", "\\.Armor$"],
  // },
  // armorBonus: {
  //   id: "armorBonus",
  //   name: "Armor Bonus",
  //   category: "defense",
  //   patterns: ["Base.Armor%6", "Armor%6", "Armor%"],
  //   canonical: "Base.Armor%6",
  //   isPercent: true,
  //   format: (v: number) => `${(v * 100).toFixed(1)}%`,
  //   description: "Armor damage reduction bonus",
  // },
  health: {
    id: "health",
    name: "Health",
    category: "defense",
    patterns: ["MaxHealth", "Health"],
    canonical: "MaxHealth",
    isPercent: false,
    format: (v: number) => `+${v.toFixed(0)}`,
    description: "Maximum health points",
    regexPatterns: ["MaxHealth", "Health$", "\\.Health$"],
  },
  healthBonus: {
    id: "healthBonus",
    name: "Health Bonus",
    category: "defense",
    patterns: ["Health%6", "Health%", "Life%6", "Life%"],
    canonical: "Health%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Maximum health bonus",
    regexPatterns: [
      "Health%6",
      "Health%",
      "\\.Health%6",
      "\\.Health%",
      "Life%6",
      "Life%",
    ],
  },
  healthRegen: {
    id: "healthRegen",
    name: "Health Regen",
    category: "defense",
    patterns: ["Base.HealthRegeneration", "HealthRegeneration", "HealthRegen"],
    canonical: "Base.HealthRegeneration",
    isPercent: false,
    format: (v: number) => `+${v.toFixed(1)}/s`,
    description: "Health regeneration per second",
  },
  // blockChance: {
  //   id: "blockChance",
  //   name: "Block Chance",
  //   category: "defense",
  //   patterns: ["BlockChance"],
  //   canonical: "BlockChance",
  //   isPercent: true,
  //   format: (v: number) => `${(v * 100).toFixed(1)}%`,
  //   description: "Chance to block attacks",
  //   regexPatterns: ["BlockChance", "Block.*Chance"],
  // },

  // ---------------------------------------------------------------------------
  // ENERGY
  // ---------------------------------------------------------------------------
  maxEnergy: {
    id: "maxEnergy",
    name: "Max Energy",
    category: "defense",
    patterns: ["Base.MaxEnergy", "MaxEnergy"],
    canonical: "Base.MaxEnergy",
    isPercent: false,
    format: (v: number) => `+${v.toFixed(0)}`,
    description: "Maximum energy points",
  },
  energyRegen: {
    id: "energyRegen",
    name: "Energy Regen",
    category: "defense",
    patterns: ["Base.EnergyRegeneration", "EnergyRegeneration", "EnergyRegen"],
    canonical: "Base.EnergyRegeneration",
    isPercent: false,
    format: (v: number) => `+${v.toFixed(1)}/s`,
    description: "Energy regeneration per second",
  },

  // ---------------------------------------------------------------------------
  // LIFESTEAL
  // ---------------------------------------------------------------------------
  // lifeSteal: {
  //   id: "lifeSteal",
  //   name: "Life Steal",
  //   category: "offense",
  //   patterns: ["Damage.LifeSteal", "LifeSteal"],
  //   canonical: "Damage.LifeSteal",
  //   isPercent: false,
  //   format: (v: number) => `+${v.toFixed(0)}`,
  //   description: "Life stolen on hit",
  //   regexPatterns: ["LifeSteal$", "\\.LifeSteal$"],
  // },
  lifeStealBonus: {
    id: "lifeStealBonus",
    name: "Life Steal Bonus",
    category: "offense",
    patterns: ["Damage.LifeSteal%6", "LifeSteal%6", "LifeSteal%", "LifeSteal"],
    canonical: "Damage.LifeSteal%6",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Life steal percentage bonus",
  },
  lifeStealChance: {
    id: "lifeStealChance",
    name: "Life Steal Chance",
    category: "offense",
    patterns: [
      "Damage.LifeStealChance%6",
      "LifeStealChance%6",
      "LifeStealChance%",
      "LifeStealChance",
      "LifeStealAmount",
    ],
    canonical: "Damage.LifeStealChance%6",
    isPercent: true,
    format: (v: number) => `${(v * 100).toFixed(1)}%`,
    description: "Chance to proc life steal",
  },

  // ---------------------------------------------------------------------------
  // RESISTANCES
  // ---------------------------------------------------------------------------
  // damageReduction: {
  //   id: "damageReduction",
  //   name: "Damage Reduction",
  //   category: "resistances",
  //   patterns: [
  //     "Base.DamageReduction%6",
  //     "DamageReduction%6",
  //     "DamageReduction%",
  //     "DamageReduction",
  //   ],
  //   canonical: "Base.DamageReduction%6",
  //   isPercent: true,
  //   format: (v: number) => `${(v * 100).toFixed(0)}%`,
  //   description: "Resistance to damage (<= 50%)",
  // },

  // ---------------------------------------------------------------------------
  // ELEMENTAL
  // ---------------------------------------------------------------------------
  fireDamageBonus: {
    id: "fireDamageBonus",
    name: "Fire Damage",
    category: "elemental",
    patterns: ["Damage.Fire%6", "Fire%6", "Fire%", "FireDamage"],
    canonical: "Damage.Fire%6",
    isPercent: true,
    format: (v: number) => `${(v * 100).toFixed(1)}%`,
    description: "Fire Damage%",
  },
  arcaneDamageBonus: {
    id: "arcaneDamageBonus",
    name: "Arcane Damage",
    category: "elemental",
    patterns: ["Damage.Arcane%6", "Arcane%6", "Arcane%", "ArcaneDamage"],
    canonical: "Damage.Arcane%6",
    isPercent: true,
    format: (v: number) => `${(v * 100).toFixed(1)}%`,
    description: "Arcane Damage%",
  },
  lightningDamageBonus: {
    id: "lightningDamageBonus",
    name: "Lightning Damage",
    category: "elemental",
    patterns: [
      "Damage.Lightning%6",
      "Lightning%6",
      "Lightning%",
      "LightningDamage",
    ],
    canonical: "Damage.Lightning%6",
    isPercent: true,
    format: (v: number) => `${(v * 100).toFixed(1)}%`,
    description: "Lightning Damage%",
  },

  // ---------------------------------------------------------------------------
  // UTILITY / OTHER
  // ---------------------------------------------------------------------------
  // xpBonus: {
  //   id: "xpBonus",
  //   name: "XP Bonus",
  //   category: "utility",
  //   patterns: ["Base.XPBonus", "XPBonus"],
  //   canonical: "Base.XPBonus",
  //   isPercent: true,
  //   format: (v: number) => `+${(v * 100).toFixed(1)}%`,
  //   description: "XP Bonus",
  //   regexPatterns: ["XPBonus$", "\\.XPBonus$"],
  // },
  // globalLoot: {
  //   id: "globalLoot",
  //   name: "Global Loot",
  //   category: "utility",
  //   patterns: ["GlobalModifiers.GlobalLoot", "GlobalLoot"],
  //   canonical: "GlobalModifiers.GlobalLoot",
  //   isPercent: true,
  //   format: (v: number) => `+${(v * 100).toFixed(0)}%`,
  //   description: "Increased loot drop chance",
  // },
  cooldownReduction: {
    id: "cooldownReduction",
    name: "Cooldown Reduction",
    category: "utility",
    patterns: [
      "CooldownReduction%",
      "CooldownReduction%6",
      "CooldownReduction",
      "EasyRPG.Attributes.CooldownReduction",
      "EasyRPG.Attributes.CooldownReduction%",
      "EasyRPG.Attributes.CooldownReduction%6",
    ],
    canonical: "CooldownReduction%",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(1)}%`,
    description: "Cooldown reduction for abilities",
  },

  // ---------------------------------------------------------------------------
  // ABILITIES (display-only, not calculated in stat totals)
  // ---------------------------------------------------------------------------
  chainLightningDamage: {
    id: "chainLightningDamage",
    name: "Chain Lightning Damage",
    category: "abilities",
    patterns: [
      "Abilities.ChainLightning.DamageMultiplier",
      "ChainLightning.DamageMultiplier",
      "ChainLightningDamage",
    ],
    canonical: "ChainLightning.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Chain Lightning damage multiplier",
    displayOnly: true,
  },
  fieryTotemDamage: {
    id: "fieryTotemDamage",
    name: "Fiery Totem Damage",
    category: "abilities",
    patterns: [
      "Abilities.FieryTotem.DamageMultiplier",
      "FieryTotem.DamageMultiplier",
      "Abilities.FieryTotem.Modifier.DamageBonus",
      "FieryTotem.Modifier.DamageBonus",
      "FieryTotemDamage",
    ],
    canonical: "FieryTotem.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Fiery Totem damage multiplier",
    displayOnly: true,
  },
  dragonFlameDamage: {
    id: "dragonFlameDamage",
    name: "Dragon Flame Damage",
    category: "abilities",
    patterns: [
      "Abilities.DragonFlame.DamageMultiplier",
      "DragonFlame.DamageMultiplier",
      "DragonFlame.DamageMulxtiplier", // Typo in game data
      "Abilities.DragonFlame.DamageMulxtiplier", // Typo in game data
      "DragonFlameDamage",
    ],
    canonical: "DragonFlame.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Dragon Flame damage multiplier",
    displayOnly: true,
  },
  enemyDeathDamage: {
    id: "enemyDeathDamage",
    name: "Death Blades Damage",
    category: "abilities",
    patterns: [
      "Abilities.EnemyDeath.DamageMultiplier",
      "EnemyDeath.DamageMultiplier",
      "EnemyDeathDamage",
    ],
    canonical: "EnemyDeath.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Death Blades damage multiplier",
    displayOnly: true,
  },
  fireOrbDamage: {
    id: "fireOrbDamage",
    name: "Fire Orb Damage",
    category: "abilities",
    patterns: [
      "Abilities.FireAtom.DamageMultiplier", // FireAtom = Fire Orb in game
      "FireAtom.DamageMultiplier",
      "FireAtomDamage",
      "FireOrbDamage",
    ],
    canonical: "FireAtom.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Fire Orb damage multiplier",
    displayOnly: true,
  },
  electricDragonsDamage: {
    id: "electricDragonsDamage",
    name: "Electric Dragons Damage",
    category: "abilities",
    patterns: [
      "Abilities.ElectricDragons.DamageMultiplier",
      "ElectricDragons.DamageMultiplier",
      "ElectricDragonsDamage",
    ],
    canonical: "ElectricDragons.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Electric Dragons damage multiplier",
    displayOnly: true,
  },
  burningShieldDamage: {
    id: "burningShieldDamage",
    name: "Burning Shield Damage",
    category: "abilities",
    patterns: [
      "Abilities.BurningShield.DamageMultiplier",
      "BurningShield.DamageMultiplier",
      "BurningShieldDamage",
    ],
    canonical: "BurningShield.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Burning Shield damage multiplier",
    displayOnly: true,
  },
  starBladesDamage: {
    id: "starBladesDamage",
    name: "Star Blades Damage",
    category: "abilities",
    patterns: [
      "Abilities.StarBlades.DamageMultiplier",
      "StarBlades.DamageMultiplier",
      "StarBladesDamage",
    ],
    canonical: "StarBlades.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Star Blades damage multiplier",
    displayOnly: true,
  },
  arcaneOrbDamage: {
    id: "arcaneOrbDamage",
    name: "Arcane Orb Damage",
    category: "abilities",
    patterns: [
      "Abilities.ArcaneOrb.DamageMultiplier",
      "ArcaneOrb.DamageMultiplier",
      "ArcaneOrbDamage",
    ],
    canonical: "ArcaneOrb.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Arcane Orb damage multiplier",
    displayOnly: true,
  },
  carnageOfFireDamage: {
    id: "carnageOfFireDamage",
    name: "Carnage of Fire Damage",
    category: "abilities",
    patterns: [
      "Abilities.CarnageOfFire.DamageMultiplier",
      "CarnageOfFire.DamageMultiplier",
      "CarnageOfFireDamage",
    ],
    canonical: "CarnageOfFire.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Carnage of Fire damage multiplier",
    displayOnly: true,
  },
  delusionOfZelkorsDamage: {
    id: "delusionOfZelkorsDamage",
    name: "Delusion of Zelkors Damage",
    category: "abilities",
    patterns: [
      "Abilities.DelusionOfZelkors.DamageMultiplier",
      "DelusionOfZelkors.DamageMultiplier",
      "DelusionOfZelkorsDamage",
    ],
    canonical: "DelusionOfZelkors.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Delusion of Zelkors damage multiplier",
    displayOnly: true,
  },
  fireBeamDamage: {
    id: "fireBeamDamage",
    name: "Fire Beam Damage",
    category: "abilities",
    patterns: [
      "Abilities.FireBeam.DamageMultiplier",
      "FireBeam.DamageMultiplier",
      "FireBeamDamage",
    ],
    canonical: "FireBeam.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Fire Beam damage multiplier",
    displayOnly: true,
  },
  sparkDamage: {
    id: "sparkDamage",
    name: "Spark Damage",
    category: "abilities",
    patterns: [
      "Abilities.Spark.DamageMultiplier",
      "Spark.DamageMultiplier",
      "SparkDamage",
    ],
    canonical: "Spark.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Spark damage multiplier",
    displayOnly: true,
  },
  vortexDamage: {
    id: "vortexDamage",
    name: "Vortex Damage",
    category: "abilities",
    patterns: [
      "Abilities.Vortex.DamageMultiplier",
      "Vortex.DamageMultiplier",
      "VortexDamage",
    ],
    canonical: "Vortex.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Vortex damage multiplier",
    displayOnly: true,
  },
  eyeOfTheStormDamage: {
    id: "eyeOfTheStormDamage",
    name: "Eye of the Storm Damage",
    category: "abilities",
    patterns: [
      "Abilities.EyeOfTheStorm.DamageMultiplier",
      "EyeOfTheStorm.DamageMultiplier",
      "EyeOfTheStormDamage",
    ],
    canonical: "EyeOfTheStorm.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Eye of the Storm damage multiplier",
    displayOnly: true,
  },
  ferocityOfWolvesDamage: {
    id: "ferocityOfWolvesDamage",
    name: "Ferocity of Wolves Damage",
    category: "abilities",
    patterns: [
      "Abilities.FerocityOfWolves.DamageMultiplier",
      "FerocityOfWolves.DamageMultiplier",
      "FerocityOfWolvesDamage",
    ],
    canonical: "FerocityOfWolves.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Ferocity of Wolves damage multiplier",
    displayOnly: true,
  },
  staticChargeDamage: {
    id: "staticChargeDamage",
    name: "Static Charge Damage",
    category: "abilities",
    patterns: [
      "Abilities.StaticCharge.DamageMultiplier",
      "StaticCharge.DamageMultiplier",
      "StaticChargeDamage",
    ],
    canonical: "StaticCharge.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Static Charge damage multiplier",
    displayOnly: true,
  },
  spinningBladeDamage: {
    id: "spinningBladeDamage",
    name: "Spinning Blade Damage",
    category: "abilities",
    patterns: [
      "Abilities.SpinningBlade.DamageMultiplier",
      "SpinningBlade.DamageMultiplier",
      "SpinningBladeDamage",
    ],
    canonical: "SpinningBlade.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Spinning Blade damage multiplier",
    displayOnly: true,
  },
  toxicityDamage: {
    id: "toxicityDamage",
    name: "Toxicity Damage",
    category: "abilities",
    patterns: [
      "Abilities.PoisonCloud.DamageMultiplier",
      "PoisonCloud.DamageMultiplier",
      "PoisonCloudDamage",
    ],
    canonical: "PoisonCloud.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Toxicity damage multiplier",
    displayOnly: true,
  },
  bloodDragonsDamage: {
    id: "bloodDragonsDamage",
    name: "Blood Dragons Damage",
    category: "abilities",
    patterns: [
      "Abilities.BloodDragons.DamageMultiplier",
      "BloodDragons.DamageMultiplier",
      "BloodDragonsDamage",
    ],
    canonical: "BloodDragons.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Blood Dragons damage multiplier",
    displayOnly: true,
  },
  arcaneApocalypseDamage: {
    id: "arcaneApocalypseDamage",
    name: "Arcane Apocalypse Damage",
    category: "abilities",
    patterns: [
      "Abilities.ArcaneApocalypse.DamageMultiplier",
      "ArcaneApocalypse.DamageMultiplier",
      "ArcaneApocalypseDamage",
    ],
    canonical: "ArcaneApocalypse.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Arcane Apocalypse damage multiplier",
    displayOnly: true,
  },
  rainOfFlamesDamage: {
    id: "rainOfFlamesDamage",
    name: "Rain of Flames Damage",
    category: "abilities",
    patterns: [
      "Abilities.FlameShowerMeteor.DamageMultiplier",
      "FlameShowerMeteor.DamageMultiplier",
      "FlameShowerMeteorDamage",
    ],
    canonical: "FlameShowerMeteor.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Rain of Flames damage multiplier",
    displayOnly: true,
  },
  arcaneTotemDamage: {
    id: "arcaneTotemDamage",
    name: "Arcane Totem Damage",
    category: "abilities",
    patterns: [
      "Abilities.ArcaneTotem.DamageMultiplier",
      "ArcaneTotem.DamageMultiplier",
      "ArcaneTotemDamage",
    ],
    canonical: "ArcaneTotem.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Arcane Totem damage multiplier",
    displayOnly: true,
  },
  lightningTotemDamage: {
    id: "lightningTotemDamage",
    name: "Lightning Totem Damage",
    category: "abilities",
    patterns: [
      "Abilities.LightningTotem.DamageMultiplier",
      "LightningTotem.DamageMultiplier",
      "LightningTotemDamage",
    ],
    canonical: "LightningTotem.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Lightning Totem damage multiplier",
    displayOnly: true,
  },
  arcaneSeedDamage: {
    id: "arcaneSeedDamage",
    name: "Arcane Seed Damage",
    category: "abilities",
    patterns: [
      "Abilities.ArcaneCrackedSeed.DamageMultiplier",
      "ArcaneCrackedSeed.DamageMultiplier",
      "ArcaneCrackedSeedDamage",
    ],
    canonical: "ArcaneCrackedSeed.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Arcane Seed damage multiplier",
    displayOnly: true,
  },
  lightningPlasmaDamage: {
    id: "lightningPlasmaDamage",
    name: "Lightning Plasma Damage",
    category: "abilities",
    patterns: [
      "Abilities.LightningPlasma.DamageMultiplier",
      "LightningPlasma.DamageMultiplier",
      "LightningPlasmaDamage",
    ],
    canonical: "LightningPlasma.DamageMultiplier",
    isPercent: true,
    format: (v: number) => `+${(v * 100).toFixed(0)}%`,
    description: "Lightning Plasma damage multiplier",
    displayOnly: true,
  },

  // ---------------------------------------------------------------------------
  // APPEND-ONLY ZONE
  // shareCodec's STAT_DICT = Object.keys(STAT_REGISTRY), so registry order is the
  // wire format for character share URLs. New stats MUST be appended here, never
  // inserted mid-registry (that shifts every later index and corrupts old links).
  // ---------------------------------------------------------------------------

  // Flat base elemental damage (ele/phys split). Resolution relies on exact
  // pattern match (which wins over the generic `damage` regex regardless of
  // order), so appending here is safe for both decoding and stat resolution.
  // elementalDamage: {
  //   id: "elementalDamage",
  //   name: "Elemental Damage (Flat)",
  //   category: "offense",
  //   patterns: ["Base.ElementalDamage", "ElementalDamage"],
  //   canonical: "Base.ElementalDamage",
  //   isPercent: false,
  //   format: (v: number) => `+${v.toFixed(0)}`,
  //   description: "Flat base elemental damage (split from physical Base.Damage)",
  //   // Anchored so the generic `damage` regex (Damage$) can never claim this.
  //   regexPatterns: ["ElementalDamage$", "\\.ElementalDamage$"],
  // },
};
